...
Our current leading theory for dark matter's role in galaxy evolution is the Dark Energy and Cold Dark Matter ($\Lambda$CDM) model (\cite{salesBaryonicSolutionsChallenges2022}). This theory provides a framework for physical simulation that incorporates a non-interacting ("cold") model for dark matter and the cosmological constant ("$\Lambda$") model of dark energy. From here, physicists are free to incorporate models for baryonic processes---the physics of ordinary matter---such as gas pressure, temperature, and star formation.

It is a common myth that if we can simulate the something, we must be able to fully understand it. Unfortunately, this is not generally true. The galaxy simulations we use are so complex and detailed that it is often very difficult to determine what physical assumptions or initial conditions cause certain behaviors. Instead, we must analyze the simulation data using similar techniques we use to analyze observational data (although it is much easier to collect data about a simulation than about a real system of galaxies). From there, we compare the results to our expected results from experimental data to test our theories.

As one would expect, the simulation data does not always line up with the observational data; instead, there are a number of known tensions between the two. The two strongest tensions are the predicted diversity of rotation curves and the satellite plane distribution. The simulations predict that AAAAAAAAAAAAAAAAAAAAAAAA

One such tension is the diversity of baryonic distribution amongst dwarf galaxies.
